...
class 语法分析器:
    ...
    @分析器母机.production('返回声明 : 动词_返回 各表达式')
    def 返回(self, 片段):
        值 = None
        if len(片段) == 2:
            if len(片段[1]) == 1:
    ...
    @分析器母机.production('片 : 表达式')
    def 索引(self, 片段):
        return 语法树.索引(值=片段[0], 片段=片段)

    @分析器母机.production('变量 : 表达式前缀 [ 片 ]')
    def 片表达式(self, 片段):
    ...
    @分析器母机.production('对于声明 : 连词_对 迭代器 : 遍历范围 块')
    def 对于声明(self, 片段):
        目标 = 片段[1]
        # TODO: 遍历 list
        目标.ctx = ast.Store()
    ...
    @分析器母机.production('对于声明 : 声明 连词_对 迭代器 连词_于 遍历范围')
    @分析器母机.production('对于声明 : 声明 连词_对 迭代器 : 遍历范围')
    def 单个对于声明(self, 片段):
        倒装 = 片段[1:]
        倒装.append([片段[0]])
        return self.对于声明(倒装)
