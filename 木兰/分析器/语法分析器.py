...
class 语法分析器:
    ...
    @分析器母机.production(语法.声明.成分(语法.类型赋值))
    ...
    @分析器母机.production(语法.类型赋值.成分(语法.名称, 冒号, 语法.类型名称, 符号_赋值, 语法.表达式))
    def 类型赋值(self, 片段):
        if 语法分析器.调试:
            print("类型赋值")
        片段[0].ctx = ast.Store()
    ...
    # TODO @分析器母机.production(语法.表达式.成分(语法.名称, 冒号, 语法.类型名称))
    def 表达式(self, 片段):
        if 语法分析器.调试:
            print("表达式")
        return 片段[0]
    ...
    def 转换为多项(self, 各实参):
        ...
        return 语法树.多项(
            上下文=ast.Load(),
            元素=[
                ast.Name((arg.arg), (ast.Load()), lineno=(arg.lineno), col_offset=(arg.col_offset)) for arg in
                各实参.args
            ],
            片段=各实参)
